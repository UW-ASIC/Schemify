...
fn hint_for(block_type: BlockType) -> PlacementHint {
    match block_type {
        ...
        BlockType::Cascode | BlockType::CascodeStack => PlacementHint {
            h_spacing: 0,
            v_spacing: 160,
...
mod tests {
    ...
    fn two_nmos_circuit(m1_nets: [&str; 3], m2_nets: [&str; 3]) -> Circuit {
    ...
    fn diff_pair_detected() {
        let circuit = two_nmos_circuit(["outm", "inp", "tail"], ["outp", "inn", "tail"]);
    ...
    fn current_mirror_detected() {
        let circuit = two_nmos_circuit(["bias", "bias", "vss"], ["out", "bias", "vss"]);
        let blocks = recognize(&circuit);
        assert_eq!(blocks.len(), 1);
    ...
    #[test]
    fn cascode_detected() {
        let circuit = two_nmos_circuit(["mid", "vb1", "vss"], ["out", "vb2", "mid"]);
        let blocks = recognize(&circuit);
        assert_eq!(blocks.len(), 1);
    ...
    fn no_false_positive_same_drains() {
        let circuit = two_nmos_circuit(["d", "g", "s"], ["d", "g", "s"]);
        let blocks = recognize(&circuit);
    ...
    fn claimed_instances_not_reused() {
        ...
        let diff_pairs: Vec<_> = blocks
            .iter()
            .filter(|b| b.block_type == BlockType::DiffPair)
            .collect();
    ...
    fn cascode_mirror_detected_over_simple_mirrors() {
        ...
        let cascode_mirrors: Vec<_> = blocks
            .iter()
        ...
        let simple_mirrors: Vec<_> = blocks
            .iter()
            .filter(|b| b.block_type == BlockType::CurrentMirror)
            .collect();
        assert!(
            simple_mirrors.is_empty(),
            "Cascode mirror should claim all 4 instances"
        );
    }
    ...
    fn multiple_blocks_detected() {
        ...
        let circuit = build_circuit(
            vec![
                make_nmos("M1"),
                make_nmos("M2"), // diff pair
                make_nmos("M3"),
                make_nmos("M4"), // current mirror
            ],
            &[
                // Diff pair: M1, M2
                ("outm", 0, 0),
