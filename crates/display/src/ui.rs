...
impl eframe::App for SchemifyGui {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // Chrome.
        components::menu_bar(ctx, app, &mut self.gui, &mut self.plugins);
        if !welcome {
            components::tab_bar(ctx, app);
        }
        components::status_bar(ctx, app, &mut self.gui);

        // Plugin panels claim side/bottom space before the CentralPanel.
        components::plugin_panels::show_panels(ctx, &mut self.plugins, &self.gui.theme);
        ...
        components::file_explorer_window(ctx, app);
        components::library_window(ctx, app, &mut self.gui);
        components::show_all_dialogs(ctx, app, &mut self.gui, &mut self.marketplace);
        components::context_menu(ctx, app, &mut self.gui);
        if !welcome && app.state.view.view_mode == ViewMode::Schematic {
            components::label_conflict_overlay(ctx, app);
        }

        // Waveform viewer — its own native window (immediate viewport).
        crate::wave_view::wave_window(ctx, app, &mut self.gui);

        // Optimizer windows — one native window per open instance.
        crate::optimizer_view::optimizer_windows(ctx, app, &mut self.gui);

        // Keyboard shortcuts (after UI so focused text edits win).
        handler::handle_shortcuts(ctx, app, &mut self.gui);
