...
class Plotter:
    ...
    def _get_legend_ncols(
        self, labels: List[str], fig: Figure, legend_placement
    ) -> int:
        """
        ...
        if end_plotting:
            if legend is not None and self.show_legend:
                ...
                if legend_eng_format and isinstance(legend[0], (int, float, np.number)):
                    formatted_legend = [f"{val:.3f} μm" for val in legend]
                else:
                    formatted_legend = [str(val) for val in legend]

                # Number of columns to use.
                ncol = self._get_legend_ncols(
                    formatted_legend, ax.figure, legend_placement
                )
