...
class LookupTableGenerator:
    ...
    def op_simulation(self):
        simulation = MosfetSimulation(self.simulator, self.model_sweeps, self.n_process)
        simulation.op_simulation()

    def build(self, filepath, filename):
        def range_args(tup):
        ...
        device_parameters = {
            key.lower(): value
            for key, value in self.simulator.device_parameters.items()
        }
        ...
        np.savez_compressed(
            os.path.join(filepath, f"{filename}.npz"),
            lookup_table=np.array(loopup_table, dtype=object),
        )
        print("Done")
