...
from .converter import import_spice
...
def main() -> None:
    ...
    parser.add_argument(
        "--stdout",
        action="store_true",
        help="Print CHN to stdout instead of writing files",
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="Output JSON format instead of native CHN",
    )
    ...
    for out in outputs:
        if args.stdout:
            print(out.to_json() if args.json else out.to_chn())
        else:
            if args.json:
                path = out.write_json(output_dir)
            else:
                path = out.write_chn(output_dir)
