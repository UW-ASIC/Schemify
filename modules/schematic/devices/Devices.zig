...
const spice = @import("simulation").SpiceIF;
...
pub const Device = struct {
    ...
    pub fn emitSpice(self: *const Device, writer: anytype, inst_name: []const u8, nets: []const []const u8, params: []const ParamOverride) !void {
        const comp = try self.toSpiceComponent(inst_name, nets, params);
        try emitComponent(writer, comp);
    }
