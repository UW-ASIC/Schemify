...
const optimizer = @import("simulation").optimizer;
...
fn runOptimizer(app: *AppState) void {
    ...
    const mosfet_kind: optimizer.MosfetKind = switch (kind) {
    ...
    var prob = optimizer.Problem{};
    var t = optimizer.Transistor{ .kind = mosfet_kind, .L = 180e-9, .gmid_min = 5.0, .gmid_max = 25.0 };
    t.setInstance(name);
    prob.transistors.append(t);

    // Add gm/Id target as objective (minimize distance from target)
    var spec = optimizer.Specification{ .kind = .minimize, .target = gmid_target };
    spec.setName("gmid");
    prob.specs.append(spec);

    const lookups = [_]optimizer.GmIdLookup{.{ .L = 180e-9 }};
    var opt = optimizer.Optimizer.init(&prob, &lookups, .{
        .max_iter = 50,
        .initial_samples = 20,
    });
