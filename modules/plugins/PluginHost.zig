...
pub const PluginHost = struct {
    ...
    getPanelHtml: *const fn (*anyopaque, u16) []const u8,
    ...
    pub inline fn panelHtml(self: PluginHost, panel_id: u16) []const u8 {
        return self.getPanelHtml(self.ctx, panel_id);
    }
...
pub fn from(comptime T: type, ptr: *T) PluginHost {
    const S = struct {
        ...
        fn getPanelHtml(c: *anyopaque, p: u16) []const u8 { return castPtr(T, c).getPanelHtml(p); }
