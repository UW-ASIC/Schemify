...
const props_mod = @import("utils.zig");
...
pub fn parse(backing: std.mem.Allocator, data: []const u8) ParseError!XSchemFiles {
    var scheme = XSchemFiles.init(backing);
    errdefer scheme.deinit();
    const arena = scheme.arena.allocator();
    ...
    while (iter.next()) |raw_line| {
        ...
        if (brace_depth > 0) {
            ...
            if (brace_depth <= 0) {
                ...
                switch (pending_tag) {
                    'v', 'V', 'E' => {},
                    'S' => parseSBlock(arena, full, &scheme),
                    'G' => parseGBlock(arena, full, &scheme) catch return error.MalformedComponent,
                    'K' => parseKBlock(arena, full, &scheme) catch return error.MalformedComponent,
                    'C' => parseComponent(arena, full, &scheme) catch return error.MalformedComponent,
                    'T' => parseText(arena, full, &scheme) catch return error.MalformedText,
                    'N' => parseWire(arena, full, &scheme) catch return error.MalformedWire,
                    'B' => try parseRect(arena, full, &scheme),
                    'L' => try parseLineShape(arena, full, &scheme),
                    'A' => try parseArc(arena, full, &scheme),
                    'P' => {}, // Polygons deferred
                    'F' => {},
        ...
        try dispatchLine(arena, line, &scheme);
    }
    return scheme;
}
...
fn dispatchLine(arena: std.mem.Allocator, line: []const u8, scheme: *XSchemFiles) ParseError!void {
    if (line.len == 0) return;
    switch (line[0]) {
        'v', 'V', 'E', '*', 'F' => {},
        'S' => parseSBlock(arena, line, scheme),
        'G' => parseGBlock(arena, line, scheme) catch return error.MalformedComponent,
        'K' => parseKBlock(arena, line, scheme) catch return error.MalformedComponent,
        'L' => try parseLineShape(arena, line, scheme),
        'B' => try parseRect(arena, line, scheme),
        'P' => {}, // Polygons deferred to Phase 4
        'A' => try parseArc(arena, line, scheme),
        'T' => parseText(arena, line, scheme) catch return error.MalformedText,
        'N' => try parseWire(arena, line, scheme),
        'C' => parseComponent(arena, line, scheme) catch return error.MalformedComponent,
        else => return error.UnknownElementTag,
    }
}
...
fn parseLineShape(arena: std.mem.Allocator, line: []const u8, scheme: *XSchemFiles) ParseError!void {
    ...
    scheme.lines.append(arena, .{ .layer = layer, .x0 = x0, .y0 = y0, .x1 = x1, .y1 = y1 }) catch
        return error.MalformedLine;
}

/// Parse B: `B layer x0 y0 x1 y1 {attrs}` -- layer 5 = pin
fn parseRect(arena: std.mem.Allocator, line: []const u8, scheme: *XSchemFiles) ParseError!void {
    var tok = tokenize(line);
    _ = tok.next();
    ...
    scheme.rects.append(arena, .{ .layer = layer, .x0 = x0, .y0 = y0, .x1 = x1, .y1 = y1 }) catch
...
fn parseArc(arena: std.mem.Allocator, line: []const u8, scheme: *XSchemFiles) ParseError!void {
    var tok = tokenize(line);
    _ = tok.next();
...
fn parseText(arena: std.mem.Allocator, line: []const u8, scheme: *XSchemFiles) ParseError!void {
...
fn parseWire(arena: std.mem.Allocator, line: []const u8, scheme: *XSchemFiles) ParseError!void {
    var tok = tokenize(line);
    _ = tok.next();
...
fn parseComponent(arena: std.mem.Allocator, line: []const u8, scheme: *XSchemFiles) ParseError!void {
    ...
    const prop_start: u32 = @intCast(scheme.props.items.len);
    ...
    if (attrs.len > 0) {
        ...
        for (result.props) |prop| scheme.props.append(arena, prop) catch return error.MalformedComponent;
        prop_count = result.count;
        for (result.props) |prop| {
            if (std.mem.eql(u8, prop.key, "name")) { inst_name = prop.value; break; }
        }
    }
    scheme.instances.append(arena, .{
...
fn parseGBlock(arena: std.mem.Allocator, line: []const u8, scheme: *XSchemFiles) !void {
    const content = extractBraceContent(line);
    if (content.len == 0) return;
    parseSymbolProperties(arena, content, scheme);
}

/// Parse K block -- sets file_type=.symbol, extracts type/format/template/extra.
fn parseKBlock(arena: std.mem.Allocator, line: []const u8, scheme: *XSchemFiles) !void {
    const content = extractBraceContent(line);
    if (content.len == 0) return;
    scheme.file_type = .symbol;
    parseSymbolProperties(arena, content, scheme);
}

/// Parse S block — raw SPICE body.  Stores the trimmed content in s_block.
fn parseSBlock(arena: std.mem.Allocator, line: []const u8, scheme: *XSchemFiles) void {
    const content = extractBraceContent(line);
    const trimmed = std.mem.trim(u8, content, " \t\r\n");
    if (trimmed.len == 0) return;
    scheme.s_block = arena.dupe(u8, trimmed) catch null;
}

/// Extract type, format, template, extra from a symbol property block.
fn parseSymbolProperties(arena: std.mem.Allocator, content: []const u8, scheme: *XSchemFiles) void {
    var ptok = props_mod.PropertyTokenizer.init(content);
    while (ptok.next()) |prop| {
        if (std.mem.eql(u8, prop.key, "type")) {
            scheme.k_type = arena.dupe(u8, prop.value) catch null;
            if (scheme.k_type != null) scheme.file_type = .symbol;
        } else if (std.mem.eql(u8, prop.key, "format"))
            scheme.k_format = arena.dupe(u8, prop.value) catch null
        else if (std.mem.eql(u8, prop.key, "template"))
            scheme.k_template = arena.dupe(u8, prop.value) catch null
        else if (std.mem.eql(u8, prop.key, "extra"))
            scheme.k_extra = arena.dupe(u8, prop.value) catch null
        else if (std.mem.eql(u8, prop.key, "global"))
            scheme.k_global = std.mem.eql(u8, prop.value, "true")
        else if (std.mem.eql(u8, prop.key, "spice_sym_def"))
            scheme.k_spice_sym_def = arena.dupe(u8, prop.value) catch null;
    }
}
