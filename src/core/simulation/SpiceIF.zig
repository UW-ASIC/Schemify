...
fn emitComponentVacask(writer: anytype, comp: SpiceComponent) !void {
    switch (comp) {
        .resistor => |r| { try writer.print("{s} ({s} {s}) resistor r=", .{ r.name, r.p, r.n }); try r.value.emit(writer); if (r.m) |m| try writer.print(" m={s}", .{m}); try writer.writeByte('\n'); },
        .capacitor => |c| { try writer.print("{s} ({s} {s}) capacitor c=", .{ c.name, c.p, c.n }); try c.value.emit(writer); if (c.ic) |ic| try writer.print(" ic={e}", .{ic}); try writer.writeByte('\n'); },
        .inductor => |l| { try writer.print("{s} ({s} {s}) inductor l=", .{ l.name, l.p, l.n }); try l.value.emit(writer); if (l.ic) |ic| try writer.print(" ic={e}", .{ic}); try writer.writeByte('\n'); },
        .diode => |d| try writer.print("{s} ({s} {s}) {s}\n", .{ d.name, d.anode, d.cathode, d.model }),
        .mosfet => |m| {
            try writer.print("{s} ({s} {s} {s} {s}) {s}", .{ m.name, m.drain, m.gate, m.source, m.bulk, m.model });
            if (m.w) |w| { try writer.writeAll(" w="); try w.emit(writer); }
        ...
        .jfet => |j| try writer.print("{s} ({s} {s} {s}) {s}\n", .{ j.name, j.drain, j.gate, j.source, j.model }),
        .independent_source => |src| try emitVacaskSource(writer, src),
        .behavioral => |b| {
        ...
        .vcvs => |e| { try writer.print("{s} ({s} {s} {s} {s}) vcvs gain=", .{ e.name, e.p, e.n, e.cp, e.cn }); try e.gain.emit(writer); try writer.writeByte('\n'); },
        .vccs => |g| { try writer.print("{s} ({s} {s} {s} {s}) vccs gain=", .{ g.name, g.p, g.n, g.cp, g.cn }); try g.gain.emit(writer); try writer.writeByte('\n'); },
        .ccvs => |h| { try writer.print("{s} ({s} {s}) ccvs vsrc=\"{s}\" gain=", .{ h.name, h.p, h.n, h.vsrc }); try h.gain.emit(writer); try writer.writeByte('\n'); },
        .cccs => |f| { try writer.print("{s} ({s} {s}) cccs vsrc=\"{s}\" gain=", .{ f.name, f.p, f.n, f.vsrc }); try f.gain.emit(writer); try writer.writeByte('\n'); },
        .subcircuit => |s| {
            try writer.print("{s} (", .{s.inst_name});
            for (s.nodes, 0..) |n, i| { if (i > 0) try writer.writeByte(' '); try writer.writeAll(n); }
            try writer.print(") {s}", .{s.name});
            for (s.params) |p| { try writer.print(" {s}=", .{p.name}); try p.value.emit(writer); }
            try writer.writeByte('\n');
        },
...
fn emitVacaskSource(writer: anytype, src: IndependentSource) !void {
    const model_name: []const u8 = switch (src.kind) { .voltage => "vsrc", .current => "isrc" };
    try writer.print("{s} ({s} {s}) {s}", .{ src.name, src.p, src.n, model_name });
    if (src.dc) |dc| try writer.print(" dc={e}", .{dc});
    if (src.ac_mag) |ac| { try writer.print(" ac={e}", .{ac}); if (src.ac_phase) |ph| try writer.print(" acphase={e}", .{ph}); }
    if (src.waveform) |wf| switch (wf) {
        .pulse => |p| try writer.print(" type=\"pulse\" v0={e} v1={e} delay={e} rise={e} fall={e} width={e} period={e}", .{ p.v1, p.v2, p.delay, p.rise, p.fall, p.width, p.period }),
        .sin => |s| { try writer.print(" type=\"sine\" ampl={e} freq={e}", .{ s.amplitude, s.freq }); if (s.offset != 0) try writer.print(" dc={e}", .{s.offset}); },
        .pwl => |pwl| { try writer.writeAll(" type=\"pwl\" wave=["); for (pwl.points, 0..) |pt, i| { if (i > 0) try writer.writeAll(", "); try writer.print("{e}, {e}", .{ pt[0], pt[1] }); } try writer.writeByte(']'); },
        .exp => |e| try writer.print(" type=\"exp\" v0={e} v1={e} td1={e} tau1={e} td2={e} tau2={e}", .{ e.v1, e.v2, e.td1, e.tau1, e.td2, e.tau2 }),
        .sffm => |s| try writer.print(" type=\"sffm\" offset={e} ampl={e} carrier={e} mod={e} signal={e}", .{ s.offset, s.amplitude, s.carrier_freq, s.mod_index, s.signal_freq }),
...
pub const Netlist = struct {
    ...
    pub fn emit(self: *const Netlist, a: Allocator, backend: Backend) ![]u8 {
        var buf = List(u8){};
        errdefer buf.deinit(a);
        try self.emitTo(buf.writer(a), backend);
        return buf.toOwnedSlice(a);
    }

    pub fn emitTo(self: *const Netlist, writer: anytype, backend: Backend) !void {
        const bh = @import("backend/lib.zig").BackendHandle.init(backend);
        try writer.print("{s}\n", .{self.title});
        ...
        for (self.params.items) |p| {
            if (p.global and backend == .xyce) try writer.print(".global_param {s}=", .{p.name}) else try writer.print(".param {s}=", .{p.name});
        ...
        for (self.components.items) |c| try bh.emitNetlistComponent(writer, c);
...
const ExprFuncMap = struct { universal: []const u8, ngspice: []const u8, xyce: []const u8, vacask: []const u8 };
...
pub fn translateFunc(comptime name: []const u8, comptime backend: Backend) []const u8 {
    for (expr_func_table) |e| if (std.mem.eql(u8, e.universal, name)) return switch (backend) { .ngspice => e.ngspice, .xyce => e.xyce, .vacask => e.vacask };
    return name;
}

// ════════════════════════════════════════���════════════════════════════════════
// ComponentType alias (used by backends)
// ══════════════════��══════════════════════════════════════════════════════════
