...
pub const Vfs = struct {
    ...
    /// Remove a file so the plugin installer can clean up stale artifacts.
    pub fn delete(path: []const u8) !void {
        if (comptime is_wasm) {
            if (wasm.vfs_file_delete(wp(path), wl(path)) < 0) return error.DeleteError;
            return;
        }
        ...
        if (comptime is_wasm) {
            ...
            var count: usize = 0;
            {
                ...
                while (it.next()) |tok| : (count += 1) {
                    if (tok.len == 0) { count -= 1; break; } // trailing NUL → empty token
                }
            }
