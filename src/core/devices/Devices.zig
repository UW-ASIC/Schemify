...
// ── Build device_table at comptime from .chn_prim files + builtin extras ──
//
// The parsed_prims array contains entries sourced from .chn_prim files.
// We convert each to a DeviceEntry, then append builtin entries that have
...
/// Builtin entries: device kinds that exist in the enum but have no .chn_prim file.
...
/// The unified device_table: first the .chn_prim-sourced entries, then builtin extras.
const device_table: []const DeviceEntry = blk: {
    @setEvalBranchQuota(10_000);
    const prim_count = primitives.prim_count;
    const total = prim_count + builtin_entries.len;
...
pub const DeviceKind = enum(u8) {
    ...
    // ── built from .chn_prim files + builtin extras)                  ── //
