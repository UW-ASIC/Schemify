...
pub const ParsedArgs = union(enum) {
    ...
    import_file: struct { path: []const u8, kind: ?ImportKind },
    import_project: []const u8,
...
const Cmd = enum { help, plugin_install, plugin_list, plugin_remove, export_svg, netlist, import_file, import_project, cmd_mode, batch_mode, commands_mode };
...
pub fn parseArgs(args: []const []const u8) ParsedArgs {
    ...
    return switch (cmd) {
        ...
        .import_file => blk: {
            ...
            const kind: ?ImportKind = if (args.len > 3) parseImportKind(args[3]) orelse break :blk .err_invalid_import_kind else null;
            break :blk .{ .import_file = .{ .path = args[2], .kind = kind } };
        },
        .import_project => if (args.len < 3) .err_missing_file else .{ .import_project = args[2] },
        .cmd_mode => blk: {
            if (args.len < 3) break :blk .err_missing_file;
...
pub fn dispatch() bool {
    ...
    switch (parseArgs(args.items)) {
        ...
        .import_file => |p| { runImport(a, p.path, p.kind); return true; },
        .import_project => |dir| { runImportProject(a, dir); return true; },
...
pub fn printHelp() void {
    ...
    out.writeAll(
        ...
        \\
        \\Import commands:
        \\  --import <file> [component|primitive|testbench]
        \\                           Import .py or .sp/.spice/.cir file as .chn.
        \\                           Type auto-detected from content if omitted.
        \\  --import-project <dir>   Import all PySpice files from a directory.
        \\
...
pub fn runImport(a: std.mem.Allocator, path: []const u8, kind: ?ImportKind) void {
    ...
    if (is_python) {
        // Recommendation #1: run actual file, not temp copy
        ...
        // Recommendation #6: accept raw SPICE files
    ...
    writeImportResults(a, result.results, kind);
}

/// Import all PySpice files from a project directory.
pub fn runImportProject(a: std.mem.Allocator, dir: []const u8) void {
    var result = import_mod.importProject(a, .{
        .pyspice = .{ .project_dir = dir },
    }) catch |e| {
    ...
    writeImportResults(a, result.results, null);
}

/// Write import results to disk, routing output by stype.
fn writeImportResults(a: std.mem.Allocator, results: []import_mod.ConvertResult, kind_override: ?ImportKind) void {
    for (results) |*r| {
        ...
        // Recommendation #4: route output based on stype
        const out_dir: []const u8 = switch (r.schemify.stype) {
            .testbench => "testbenches",
            .primitive => "schematics",
            else => "schematics",
        };

        // Ensure output directory exists
        utility.platform.fs.cwd().makePath(out_dir) catch {};

        var out_buf: [512]u8 = undefined;
        const out_path = std.fmt.bufPrint(&out_buf, "{s}/{s}.chn", .{ out_dir, r.name }) catch {
