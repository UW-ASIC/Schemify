...
pub fn execInfo(
    ...
    if (std.mem.eql(u8, args[0], "exists")) {
        ...
        if (std.mem.startsWith(u8, name, "env(") and std.mem.endsWith(u8, name, ")")) {
            ...
            return if (platform.getEnv(env_name) != null) "1" else "0";
        }
...
pub fn readSourceFile(path: []const u8, allocator: std.mem.Allocator) ?[]const u8 {
    return platform.fs.cwd().readFileAlloc(allocator, path, 10 * 1024 * 1024) catch null;
}

fn isDirExists(path: []const u8) bool {
    var dir = platform.fs.cwd().openDir(path, .{}) catch return false;
    dir.close();
    return true;
}

fn isFileExists(path: []const u8) bool {
    platform.fs.cwd().access(path, .{}) catch return false;
    return true;
}
