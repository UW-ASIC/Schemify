...
const Prop = std.meta.Elem(@TypeOf(@as(Schemify.ComponentDesc, undefined).props));
...
pub fn mapXSchemToSchemify(
    ...
    if (schematic.s_block) |sb| {
        sfy.spice_body = try sfy.strings.add(a, sb);
    }
...
fn mapWires(sfy: *Schemify, a: Allocator, src: *const XSchemFiles) Allocator.Error!void {
    ...
    for (0..src.wires.len) |i| {
        ...
        _ = try sfy.addWireWithNet(a, x0, y0, x1, y1, sl.items(.net_name)[i] orelse "");
    }
}
...
fn mapTexts(sfy: *Schemify, a: Allocator, src: *const XSchemFiles) Allocator.Error!void {
    ...
    for (0..src.texts.len) |i| {
        const content_ref = try sfy.strings.add(a, sl.items(.content)[i]);
        try sfy.texts.append(a, .{
            .content = content_ref,
            .x = f2i(sl.items(.x)[i]),
            .y = f2i(sl.items(.y)[i]),
            .layer = layerU8(sl.items(.layer)[i]),
...
fn mapKBlock(sfy: *Schemify, a: Allocator, sym: *const XSchemFiles) Allocator.Error!void {
    ...
    if (sym.k_spice_sym_def) |ssd| sfy.spice_sym_def = try sfy.strings.add(a, ssd);
}
...
fn mapInstances(sfy: *Schemify, a: Allocator, src: *const XSchemFiles, sym_resolver: ?SymResolver) Allocator.Error!void {
    ...
    for (0..src.instances.len) |i| {
        ...
        if (is_stem_label) {
            ...
            if (lab.len > 0) {
                _ = try sfy.addWireWithNet(a, inst_x, inst_y, inst_x, inst_y, lab);
            }
        }
            ...
            if (resolver.resolve(raw_sym)) |resolved_sym| {
                ...
                if (resolved_sym.pins.len > 0) {
                    ...
                    for (0..resolved_sym.pins.len) |pi| {
                        pin_refs[pi] = .{
                            .name = try sfy.strings.add(a, pin_sl.items(.name)[pi]),
                ...
                if (resolved_sym.k_format) |fmt| sd.format = try sfy.strings.add(a, fmt);
                if (resolved_sym.k_template) |tmpl| sd.template = try sfy.strings.add(a, tmpl);
...
fn mapPortPins(sfy: *Schemify, a: Allocator) Allocator.Error!void {
    ...
    for (0..sfy.instances.len) |i| {
        ...
        const lab = findSfyPropValue(sfy, inst_props, "lab");
    ...
    for (0..n) |i| {
        ...
        if (splitBusPin(e.name)) |parts| {
            ...
            if (count == width and width > 1) {
                ...
                try sfy.drawPinStr(a, parts.base, e.x, e.y, e.dir);
                sfy.pins.items(.width)[sfy.pins.len - 1] = @intCast(width);
                continue;
            }
        }

        consumed[i] = true;
        try sfy.drawPinStr(a, e.name, e.x, e.y, e.dir);
        if (e.num) |pn| sfy.pins.items(.num)[sfy.pins.len - 1] = @as(u16, @intCast(pn));
    }
}
...
fn findSfyPropValue(sfy: *const Schemify, props: []const core.types.Property, key: []const u8) []const u8 {
    for (props) |p| {
        if (std.mem.eql(u8, sfy.str(p.key), key)) return sfy.str(p.val);
    }
    return "";
}
...
// -- Schemify -> XSchemFiles (export) moved to exporter.zig ----------------
