...
const Tcl = @import("TCL/mod.zig").Tcl;
...
fn seedDefaults(tcl: *Tcl) !void {
    ...
    if (platform.getEnv("USER_CONF_DIR")) |ucd| {
        try tcl.setVar("USER_CONF_DIR", ucd);
    } else if (platform.homeDir()) |home| {
    ...
    if (platform.getEnv("PDK_ROOT")) |pdk| {
...
fn probeShareDir() ?[]const u8 {
    ...
    for (&candidates) |path| {
        platform.fs.cwd().access(path, .{}) catch continue;
        return path;
    }
    return null;
}
