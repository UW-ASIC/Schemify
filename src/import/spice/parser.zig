...
pub const Include = struct {
    path: []const u8,
    section: ?[]const u8 = null, // for .lib "file" "corner"
};
...
pub fn parseNetlist(arena: Allocator, source: []const u8) !Netlist {
    ...
    for (lines[1..]) |line| {
        ...
        if (lo0 == '.') {
            ...
            } else if (std.mem.eql(u8, cmd, ".include") or std.mem.eql(u8, cmd, ".inc")) {
                if (toks.len >= 2) {
                    const path = stripQuotes(toks[1]);
                    try includes.append(arena, .{ .path = path });
                }
            } else if (std.mem.eql(u8, cmd, ".lib")) {
                if (toks.len >= 2) {
                    const path = stripQuotes(toks[1]);
                    const section = if (toks.len >= 3) stripQuotes(toks[2]) else null;
                    try includes.append(arena, .{ .path = path, .section = section });
                }
    ...
    return .{
        ...
        .includes = try arena.dupe(Include, includes.items),
    };
}
...
/// Strip surrounding double or single quotes from a token.
...
test "parseNetlist — .include directives" {
    const source =
        \\* Test with includes
        \\.include "sky130.lib"
        \\.include 'models/nmos.sp'
        \\.lib "corners.lib" "tt"
        \\.subckt inv in out vdd vss
        \\M1 out in vdd vdd pfet W=1u L=180n
        \\.ends inv
        \\.end
    ;
    ...
    try std.testing.expectEqual(@as(usize, 3), netlist.includes.len);
    try std.testing.expectEqualStrings("sky130.lib", netlist.includes[0].path);
    try std.testing.expect(netlist.includes[0].section == null);
    try std.testing.expectEqualStrings("models/nmos.sp", netlist.includes[1].path);
    try std.testing.expectEqualStrings("corners.lib", netlist.includes[2].path);
    try std.testing.expectEqualStrings("tt", netlist.includes[2].section.?);
    try std.testing.expectEqual(@as(usize, 1), netlist.subckts.len);
}

test "parseNetlist — .include with no path" {
    const source =
        \\* Bad include
        \\.include
        \\.end
    ...
    const netlist = try parseNetlist(arena, source);
    try std.testing.expectEqual(@as(usize, 0), netlist.includes.len);
}
