...
const WasmDir = struct {
    ...
    pub fn access(self: WasmDir, sub_path: []const u8, _: anytype) !void {
        var path_buf: [1024]u8 = undefined;
        const full = self.joinPath(&path_buf, sub_path) orelse return error.FileNotFound;
        if (vfs_file_len(full.ptr, @intCast(full.len)) < 0) return error.FileNotFound;
    }

    pub fn createFile(self: WasmDir, sub_path: []const u8, _: anytype) !WasmFile {
        var path_buf: [1024]u8 = undefined;
        const full = self.joinPath(&path_buf, sub_path) orelse return error.FileNotFound;
    ...
    pub fn deleteFile(self: WasmDir, sub_path: []const u8) !void {
        var path_buf: [1024]u8 = undefined;
        const full = self.joinPath(&path_buf, sub_path) orelse return error.FileNotFound;
        _ = vfs_file_delete(full.ptr, @intCast(full.len));
    }

    pub fn makePath(_: WasmDir, _: []const u8) !void {}
...
/// Not available on WASM — use AsyncHttpGet instead.
pub fn httpGetSync(alloc: std.mem.Allocator, url: []const u8) ![]u8 {
    if (comptime is_wasm) return error.HttpRequestFailed;
...
// ── Async HTTP (WASM only) ───────────────────────────────────────────────────
// Start a fetch, poll each frame. JS host manages the actual HTTP request.

pub const AsyncHttpGet = struct {
    req_id: i32,
    ...
    pub fn start(url: []const u8) AsyncHttpGet {
        if (comptime is_wasm) {
            const id = nextWasmReqId();
            platform_http_get_start(url.ptr, @intCast(url.len), id);
            return .{ .req_id = id };
        }
        return .{ .req_id = -1 };
    }

    /// Poll for response. On success, copies into `buf` and returns the slice.
    /// Returns `.pending` if still in flight, `.failed` on error.
    pub fn poll(self: AsyncHttpGet, buf: []u8) PollResult {
        if (comptime is_wasm) {
            const n = platform_http_get_poll(self.req_id, buf.ptr, @intCast(buf.len));
            if (n == -1) return .pending;
            if (n < 0) return .failed;
            return .{ .done = buf[0..@as(usize, @intCast(n))] };
        }
        return .failed;
    }
};

var wasm_req_counter: i32 = 1;
fn nextWasmReqId() i32 {
    const id = wasm_req_counter;
    wasm_req_counter +%= 1;
    return id;
}

extern "host" fn platform_http_get_start(url_ptr: [*]const u8, url_len: u32, req_id: i32) void;
extern "host" fn platform_http_get_poll(req_id: i32, buf_ptr: [*]u8, buf_len: u32) i32;

// ── Environment ──────────────────────────────────────────────────────────────

/// Portable getenv. Returns null on WASM (no process environment).
