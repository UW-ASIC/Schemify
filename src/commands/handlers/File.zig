...
pub fn handleImmediate(imm: Immediate, state: anytype) Error!void {
    switch (imm) {
        ...
        .close_tab => {
            ...
            const closing = &state.documents.items[idx];
            // Deindex testbench before closing.
            if (closing.sch.stype == .testbench) {
                if (closing.origin == .chn_file) state.tb_index.deindexTb(closing.origin.chn_file);
            }
            // Remember path for reopen_last_closed.
            switch (closing.origin) {
