...
pub fn handle(imm: Immediate, state: anytype) Error!void {
    switch (imm) {
        ...
        .highlight_selected_nets => {
            ...
            try fio.selection.wires.resize(alloc, sch.wires.len, false);
        ...
        .unhighlight_selected_nets => {
            ...
            const fio = state.active() orelse return;
            var it = fio.selection.wires.iterator(.{});
            while (it.next()) |wi| {
            ...
            for (0..sch.instances.len) |ii| {
                ...
                for (0..sch.wires.len) |wi| {
                    ...
                    if (ptEq(ws, ip) or ptEq(we, ip)) {
                        setBit(&fio.selection.wires, alloc, wi) catch continue;
                    }
                }
            }
    ...
    while (round < 8) : (round += 1) {
        ...
        for (0..sch.wires.len) |a| {
            ...
            for (0..sch.wires.len) |b| {
                ...
                if (shares) {
                    setBit(&fio.selection.wires, alloc, b) catch continue;
                    added = true;
                }
            }
        }
...
fn highlightDupRefdes(state: anytype) void {
    ...
    for (0..sch.instances.len) |i| {
        ...
        (map.getOrPutValue(alloc, inst.name, 0) catch continue).value_ptr.* += 1;
    }
    fio.selection.clear();
    for (0..sch.instances.len) |i| {
        const inst = sch.instances.get(i);
        if ((map.get(inst.name) orelse 0) > 1) {
            setBit(&fio.selection.instances, alloc, i) catch continue;
        }
    }
...
fn renameDupRefdes(state: anytype) Error!void {
    ...
    for (0..sch.instances.len) |i| {
        const inst = sch.instances.get(i);
        const res = try map.getOrPut(alloc, inst.name);
