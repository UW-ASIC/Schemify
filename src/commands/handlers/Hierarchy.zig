...
pub fn handleHierarchy(imm: Immediate, state: anytype) void {
    switch (imm) {
        ...
        .edit_in_new_tab => {
            ...
            const sym = fio.sch.str(inst.symbol);
            var buf: [512]u8 = undefined;
            const path = resolveSymbolFile(state, fio, sym, ".chn", &buf) orelse
                resolveSymbolFile(state, fio, sym, ".chn_prim", &buf) orelse {
...
fn descendInto(state: anytype, comptime ext: []const u8, comptime kind: []const u8) void {
    ...
    const sym = fio.sch.str(inst.symbol);
    var buf: [512]u8 = undefined;
    const path = resolveSymbolFile(state, fio, sym, ext, &buf) orelse {
...
fn makeSchematicFromSymbol(state: anytype) void {
    ...
    if (sch.pins.len > 0) {
        ...
        for (0..sch.pins.len) |i| {
            ...
            w.print("    {s}  {s}  x={d}  y={d}\n", .{ sch.str(pn[i]), dir_str, px[i], py[i] }) catch return;
        }
    }
    ...
    for (0..sch.pins.len) |i| {
        ...
        w.print("    {s}  {s}  x={d}  y={d}\n", .{
            sch.str(pn[i]), kind_str, px[i], py[i],
        }) catch return;
    }
