...
pub fn handleView(imm: Immediate, state: anytype) void {
    switch (imm) {
        ...
        .view_doc => {
            ...
            state.setStatus("Documentation view");
        },
        .export_svg => {
...
fn exportSvgFile(sch: anytype, path: []const u8) !void {
    ...
    for (0..sch.instances.len) |i| {
        ...
        if (prim) |entry| {
            ...
            try svgWriteGenericBox(sch, sd, ox, oy, rot, flip, file, &buf);
        }

        // Instance name label.
        const name_str = sch.str(inst_name[i]);
        if (name_str.len > 0) {
            const rf = svgRotFlip(25, -20, rot, flip);
            len = (std.fmt.bufPrint(&buf, "<text class=\"n\" x=\"{d}\" y=\"{d}\">{s}</text>\n", .{ ox + rf[0], oy + rf[1], name_str }) catch &buf).len;
            try file.writeAll(buf[0..len]);
        }
    }
...
fn svgWriteGenericBox(sch: anytype, sd: anytype, ox: i32, oy: i32, rot: u2, flip: bool, file: anytype, buf: *[512]u8) !void {
    var len: usize = 0;
    const half_w: i32 = 25;
    const half_h: i32 = 25;
    ...
    for (sd.pins) |pin| {
        ...
        try file.writeAll(buf[0..len]);
        const pn_str = sch.str(pin.name);
        if (pn_str.len > 0) {
            const lx = if (pin.x < 0) ox + ep[0] + 3 else ox + ep[0] - @as(i32, @intCast(pn_str.len)) * 6 - 3;
            len = (std.fmt.bufPrint(buf, "<text class=\"p\" x=\"{d}\" y=\"{d}\">{s}</text>\n", .{ lx, oy + ep[1] - 2, pn_str }) catch buf).len;
            try file.writeAll(buf[0..len]);
        }
    }
}
