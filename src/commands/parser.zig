//! Command text parser — converts text commands into typed Command values.
//!
//! Every Immediate/Undoable variant without a payload is automatically
//! available by its union tag name (underscores).  Kebab-case aliases
//! are derived at runtime by replacing '-' → '_'.  A small manual alias
//! table provides classic vim-bar short-forms.
//!
//! Parameterised commands and meta-commands are parsed by hand.
...
fn parseSim(rest: []const u8) Result {
    const backend: types.SimBackend = if (rest.len == 0) .ngspice else if (eql(rest, "xyce")) .xyce else if (eql(rest, "vacask")) .vacask else .ngspice;
    return .{ .command = .{ .undoable = .{ .run_sim = .{ .sim = backend } } } };
}
...
pub fn printCommandList(file: anytype) void {
    // Use writeAll throughout since dvui.fs.File has no .print().
    file.writeAll(
        \\
        \\Headless CLI:
