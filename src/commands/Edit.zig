...
pub fn handleUndoable(und: Undoable, state: anytype) Error!void {
    switch (und) {
        ...
        .delete_selected => {
            ...
            var si: usize = 0;
            var sw: usize = 0;
            for (0..sch.instances.len) |i| if (selInst(state, i)) {
                si += 1;
            };
            for (0..sch.wires.len) |i| if (selWire(state, i)) {
                sw += 1;
            };
            ...
            sw = 0;
            for (0..sch.instances.len) |i| {
                if (selInst(state, i) and si < snap_inst.len) {
                    snap_inst[si] = sch.instances.get(i);
                    si += 1;
                }
            }
            for (0..sch.wires.len) |i| {
                if (selWire(state, i) and sw < snap_wire.len) {
                    snap_wire[sw] = sch.wires.get(i);
                    sw += 1;
                }
            }
            ...
            while (wi > 0) {
                wi -= 1;
                if (selWire(state, wi)) sch.wires.orderedRemove(wi);
            }
            var ii = sch.instances.len;
            while (ii > 0) {
                ii -= 1;
                if (selInst(state, ii)) sch.instances.orderedRemove(ii);
            }
        ...
        .delete_device => |p| {
            ...
            state.history.push(state.allocator(), .{ .delete_device = .{ .sym_path = inst.symbol, .name = inst.name, .pos = .{ inst.x, inst.y } } });
