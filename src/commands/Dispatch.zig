//! Central command dispatcher — routes Commands to handler functions.
//! The `State` parameter is duck-typed: any pointer whose child exposes
//! the fields/methods used by handlers (active(), setStatus(), etc.).
...
fn dispatchUndoable(und: Undoable, state: anytype) DispatchError!void {
    // Record in undo history for commands with simple inverses
    if (state.active()) |fio| {
        if (h.invertCommand(und) != null) {
            fio.undo_history.push(und);
