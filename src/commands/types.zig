...
//!
//! EXHAUSTIVE: Every action the GUI and plugins can trigger is represented here.
//! Plugins can extend the system via `plugin_command` (Immediate) and
//! `plugin_mutation` (Undoable) with arbitrary tag/payload strings.
...
/// Simulation backend selector — mirrors core.SpiceIF.Backend.
pub const SimBackend = enum { ngspice, xyce, vacask };
...
// ── GUI-only actions (consumed by the shell, never queued) ───────────────────
