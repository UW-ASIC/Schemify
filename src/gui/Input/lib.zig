...
fn handleTextInput(app: *AppState, code: dvui.enums.Key, ctrl: bool, shift: bool) bool {
    _ = ctrl;
    ...
    switch (code) {
        ...
        .enter => {
            if (draw.text_len > 0) {
                if (draw.text_pos) |pos| {
                    actions.enqueue(app, .{ .undoable = .{ .add_text = .{
                        .content = draw.text_buf[0..draw.text_len],
                        .x = pos[0], .y = pos[1],
                    } } }, "Text placed");
                }
            }
        ...
        .backspace => {
            if (draw.text_len > 0) draw.text_len -= 1;
            return true;
        },
        else => {
            const ch = km.keyToChar(code, shift);
            if (ch != 0 and draw.text_len < draw.text_buf.len - 1) {
                draw.text_buf[draw.text_len] = ch;
                draw.text_len += 1;
            }
            return true;
        },
