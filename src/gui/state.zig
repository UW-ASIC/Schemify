//! GUI state — single consolidated module.
//!
//! Merges types.zig + AppState.zig + Document.zig + TbIndex.zig + lib.zig
//! from the original state/ directory into one file.
...
pub const Sim = core.simulation.SpiceIF.Backend;
...
pub const Document = struct {
    ...
    pub fn open(a: std.mem.Allocator, logger: ?*utility.Logger, path: []const u8) !Document {
        const data = try dvui.fs.cwd().readFileAlloc(a, path, std.math.maxInt(usize));
        defer a.free(data);
    ...
    pub fn createNetlist(self: *Document, sim: Sim) ![]u8 {
        return self.sch.emitSpice(self.alloc, sim, null);
    }
    ...
    pub fn runSpiceSim(self: *Document, sim: Sim, netlist_content: []const u8) !std.process.Child.Term {
        ...
        const base_name = switch (self.origin) { .chn_file => |p| std.fs.path.stem(p), else => self.name };
        ...
        dvui.fs.cwd().writeFile(.{ .sub_path = sp_path, .data = netlist_content }) catch return error.WriteFailed;
        const sim_bin: []const u8 = switch (sim) { .ngspice => "ngspice", .xyce => "xyce", .vacask => "vacask" };
        var cmd_buf: [1024]u8 = undefined;
        const cmd_str = std.fmt.bufPrint(&cmd_buf, "{s} -b \"{s}\" -r \"{s}\"; echo '--- Done. Press Enter to close. ---'; read", .{ sim_bin, sp_path, raw_path }) catch return error.Overflow;
        var child = std.process.Child.init(&.{ "xterm", "-T", "Schemify Simulator", "-e", "sh", "-c", cmd_str }, self.alloc);
    ...
    pub fn saveAsChn(self: *Document, path: []const u8) !void {
        ...
        try dvui.fs.cwd().writeFile(.{ .sub_path = path, .data = out });
...
pub const AppState = struct {
    ...
    fn buildTbIndex(self: *AppState) void {
        ...
        for (self.config.paths.chn_tb) |tb_path| {
            const data = dvui.fs.cwd().readFileAlloc(a, tb_path, std.math.maxInt(usize)) catch continue;
            defer a.free(data);
    ...
    pub fn setActiveIdx(self: *AppState, idx: u32) void {
        if (idx != self.active_idx) self.active_idx = idx;
    }
};
