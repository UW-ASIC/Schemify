...
fn drawTitle(text_bright: dvui.Color, text_dim: dvui.Color) void {
    const title_font = dvui.Font.find(.{ .family = "Vera", .size = 32 });
    const subtitle_font = dvui.Font.find(.{ .family = "Vera", .size = 14 });
...
fn drawQuickActions(
    ...
    const section_font = dvui.Font.find(.{ .family = "Vera", .size = 13 });
    ...
    // New Primitive
    if (dvui.button(@src(), "New Primitive", .{}, .{
        .padding = .{ .x = 16, .y = 10, .w = 16, .h = 10 },
        .color_fill = card_bg,
        .color_text = text_bright,
        .corner_radius = .{ .x = 6, .y = 6, .w = 6, .h = 6 },
    })) {
        app.queue.push(alloc, .{ .immediate = .open_new_prim_dialog }) catch {};
    }
}
...
fn drawRecentFiles(
    ...
    _ = text_bright;
    const section_font = dvui.Font.find(.{ .family = "Vera", .size = 13 });
    const body_font = dvui.Font.find(.{ .family = "Vera", .size = 12 });
    const small_font = dvui.Font.find(.{ .family = "Vera", .size = 10 });
...
fn drawHints(text_dim: dvui.Color) void {
    const hint_font = dvui.Font.find(.{ .family = "Vera", .size = 11 });
