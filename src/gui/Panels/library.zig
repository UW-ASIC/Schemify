...
pub fn draw(app: *AppState) void {
    ...
    var fwin = dvui.floatingWindow(@src(), .{
        ...
        .min_size_content = .{ .w = 380, .h = 300 }, .background = true,
...
fn placeSelected(app: *AppState) void {
    ...
    app.tool.placement = st.Placement.fromName(prim.kind_name);
    app.tool.active = .select;
    app.status_msg = "Click to place — right-click or Esc to cancel";
}
