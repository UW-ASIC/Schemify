...
const export_items = [_]MenuItem{
    .{ .action = .{ .label = "Export PDF", .cmd = .{ .immediate = .export_pdf      }, .msg = "Export PDF (stub)" } },
    .{ .action = .{ .label = "Export PNG", .cmd = .{ .immediate = .export_png      }, .msg = "Export PNG (stub)" } },
    .{ .action = .{ .label = "Export SVG", .cmd = .{ .immediate = .export_svg      }, .msg = "Export SVG (stub)" } },
    .{ .action = .{ .label = "Screenshot", .cmd = .{ .immediate = .screenshot_area }, .msg = "Screenshot (stub)" } },
};
...
pub fn draw(app: *AppState) void {
    ToolbarBar.draw(drawMenus, app, 0);
}
...
fn drawMenus(app: *AppState) void {
    ...
    // File menu
    if (dvui.menuItemLabel(@src(), "File", .{ .submenu = true }, .{ .id_extra = 1 })) |r| {
        var fw = dvui.floatingMenu(@src(), .{ .from = r }, .{ .id_extra = 1 });
        defer fw.deinit();
        renderItems(app, fw, &file_items);
    }

    // Edit menu
    if (dvui.menuItemLabel(@src(), "Edit", .{ .submenu = true }, .{ .id_extra = 2 })) |r| {
        var fw = dvui.floatingMenu(@src(), .{ .from = r }, .{ .id_extra = 2 });
        defer fw.deinit();
        renderItems(app, fw, &edit_items);
    }

    // View menu (grid toggle has a dynamic label, handled inline)
    if (dvui.menuItemLabel(@src(), "View", .{ .submenu = true }, .{ .id_extra = 3 })) |r| {
        var fw = dvui.floatingMenu(@src(), .{ .from = r }, .{ .id_extra = 3 });
        defer fw.deinit();
        renderItems(app, fw, &view_items);
        ...
        renderItems(app, fw, &view_items_after_grid);
    }

    // Wire/Draw menu
    if (dvui.menuItemLabel(@src(), "Wire/Draw", .{ .submenu = true }, .{ .id_extra = 4 })) |r| {
        var fw = dvui.floatingMenu(@src(), .{ .from = r }, .{ .id_extra = 4 });
        defer fw.deinit();
        renderItems(app, fw, &wire_draw_items);
    }

    // Hierarchy menu
    if (dvui.menuItemLabel(@src(), "Hierarchy", .{ .submenu = true }, .{ .id_extra = 5 })) |r| {
        var fw = dvui.floatingMenu(@src(), .{ .from = r }, .{ .id_extra = 5 });
        defer fw.deinit();
        renderItems(app, fw, &hierarchy_items);
    }

    // Netlist menu
    if (dvui.menuItemLabel(@src(), "Netlist", .{ .submenu = true }, .{ .id_extra = 6 })) |r| {
        var fw = dvui.floatingMenu(@src(), .{ .from = r }, .{ .id_extra = 6 });
        defer fw.deinit();
        renderItems(app, fw, &netlist_items);
    }

    // Sim menu
    if (dvui.menuItemLabel(@src(), "Sim", .{ .submenu = true }, .{ .id_extra = 7 })) |r| {
        var fw = dvui.floatingMenu(@src(), .{ .from = r }, .{ .id_extra = 7 });
        defer fw.deinit();
        renderItems(app, fw, &sim_items);
    }

    // Export menu
    if (dvui.menuItemLabel(@src(), "Export", .{ .submenu = true }, .{ .id_extra = 8 })) |r| {
        var fw = dvui.floatingMenu(@src(), .{ .from = r }, .{ .id_extra = 8 });
        defer fw.deinit();
        renderItems(app, fw, &export_items);
    }

    // Transform menu
    if (dvui.menuItemLabel(@src(), "Transform", .{ .submenu = true }, .{ .id_extra = 9 })) |r| {
        var fw = dvui.floatingMenu(@src(), .{ .from = r }, .{ .id_extra = 9 });
        defer fw.deinit();
        renderItems(app, fw, &transform_items);
    }

    // Plugins menu (dynamic: lists loaded panels)
    if (dvui.menuItemLabel(@src(), "Plugins", .{ .submenu = true }, .{ .id_extra = 10 })) |r| {
...
fn renderItems(app: *AppState, fw: *dvui.FloatingMenuWidget, comptime items: []const MenuItem) void {
    inline for (items, 0..) |entry, idx| {
        switch (entry) {
            .action => |a| {
                if (menuAction(a.label, idx)) {
                    actions.enqueue(app, a.cmd, a.msg);
                    fw.close();
                }
            },
            .gui_cmd => |g| {
                if (menuAction(g.label, idx)) {
                    actions.runGuiCommand(app, g.cmd);
                    fw.close();
                }
            },
            .separator => {
                _ = dvui.separator(@src(), .{ .id_extra = idx });
            },
        }
    }
}
