...
fn resolveSubcktSymbol(symbol: []const u8, doc_origin: st.Origin, project_dir: []const u8, doc: *st.Document) ?*const SubcktSymbol {
    ...
    const data = dvui.fs.cwd().readFileAlloc(arena, chn_path, std.math.maxInt(usize)) catch return null;
...
pub fn draw(ctx: *const RenderContext, sch: *const Schemify, app: *st.AppState, sel: *const st.Selection, hide_port_prims: bool) void {
    ...
    for (0..sch.instances.len) |i| {
        ...
        if (prim) |entry| {
            ...
            const subckt = if (active_doc) |doc| resolveSubcktSymbol(isymbol[i], doc_origin, app.project_dir, doc) else null;
            if (subckt) |s| {
...
fn drawSubcktBox(sym: *const SubcktSymbol, origin: Vec2, rot: u2, flip: bool, vp: RenderViewport, color: Color, batch: *h.LineBatch, labels: *h.LabelList, lalloc: Allocator) void {
...
fn drawGenericBox(origin: Vec2, rot: u2, flip: bool, vp: RenderViewport, color: Color, batch: *h.LineBatch, sd: core.types.SymData, labels: *h.LabelList, lalloc: Allocator) void {
    const s: Vec2 = @splat(vp.scale);
    const sym_w: f32 = @max(0.9, 1.8 * vp.scale);
    ...
    for (sd.pins, 0..) |pin, pi| {
        ...
        if (vp.scale >= 0.3 and pin.name.len > 0) {
            ...
            h.queueLabel(labels, lalloc, pin.name, label_x, ep_px[1] - 8.0 * vp.scale, color, 0x9000 + pi);
        }
    }
}
