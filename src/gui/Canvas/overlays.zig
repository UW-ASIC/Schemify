...
//! testbench linkage button strip + ghost wire overlay, and
//! spice2schematic ghost geometry from simulation results.
...
const spice2sch = @import("import").spice;
...
// ═══════════════════════════════════════════════════════════════════════════
// Placement ghost (component follows cursor before being placed)
// ═══════════════════════════════════════════════════════════════════════════

pub fn drawPlacementGhost(ctx: *const RenderContext, app: *st.AppState) void {
    const pl = app.tool.placement orelse return;
    const kind_name = pl.kindSlice();
    const entry = core.devices.primitives.findByNameRuntime(kind_name) orelse return;
    ...
    if (entry.hasDrawing()) {
        ...
        const circles = entry.drawCircles();
...
// ═══════════════════════════════════════════════════════════════════════════
// Drawing tool previews (line, rect, circle, arc, polygon, text)
// ═══════════════════════════════════════════════════════════════════════════

pub fn drawDrawingToolPreview(ctx: *const RenderContext, app: *st.AppState) void {
    const vp = ctx.vp;
    const pal = ctx.pal;
    const draw = &app.tool.draw;
    const cursor = app.gui.hot.canvas.cursor_world;
    const preview_col = Color{ .r = pal.wire_preview.r, .g = pal.wire_preview.g, .b = pal.wire_preview.b, .a = 180 };
    const thickness: f32 = 1.5;

    switch (app.tool.active) {
        .line => {
            if (draw.first_point) |fp| {
                const start = h.w2p(fp, vp);
                const end = h.w2p(cursor, vp);
                h.strokeDot(start, types.wire_preview_dot_radius, preview_col);
                h.strokeLine(start[0], start[1], end[0], end[1], thickness, preview_col);
                h.strokeDot(end, types.wire_endpoint_radius, preview_col);
            }
        },
        .rect => {
            if (draw.first_point) |fp| {
                const tl = h.w2p(.{ @min(fp[0], cursor[0]), @min(fp[1], cursor[1]) }, vp);
                const br = h.w2p(.{ @max(fp[0], cursor[0]), @max(fp[1], cursor[1]) }, vp);
                h.strokeRectOutline(tl, br, thickness, preview_col);
            }
        },
        .circle => {
            if (draw.first_point) |fp| {
                const center = h.w2p(fp, vp);
                const dx: f64 = @floatFromInt(cursor[0] - fp[0]);
                const dy: f64 = @floatFromInt(cursor[1] - fp[1]);
                const radius_world: f32 = @floatCast(@sqrt(dx * dx + dy * dy));
                const radius_px = radius_world * vp.scale;
                if (radius_px > 1.0) {
                    h.strokeCircle(center, radius_px, thickness, preview_col);
                }
                h.strokeDot(center, types.wire_preview_dot_radius, preview_col);
            }
        },
        .arc => {
            if (draw.first_point) |fp| {
                ...
                switch (draw.arc_step) {
                    .center => {},
                    .radius_start => {
                        // Show line from center to cursor (radius preview).
                        const edge = h.w2p(cursor, vp);
                        h.strokeLine(center[0], center[1], edge[0], edge[1], thickness, preview_col);
                    },
                    .sweep => {
                        // Show arc from start point to cursor.
                        const start_pt = draw.arc_second orelse return;
                        const dx1: f64 = @floatFromInt(start_pt[0] - fp[0]);
                        const dy1: f64 = @floatFromInt(start_pt[1] - fp[1]);
                        const radius_world: f32 = @floatCast(@sqrt(dx1 * dx1 + dy1 * dy1));
                        const radius_px = radius_world * vp.scale;
                        const start_angle: i16 = @intFromFloat(@round(std.math.atan2(
                            -@as(f64, @floatFromInt(start_pt[1] - fp[1])),
                            @as(f64, @floatFromInt(start_pt[0] - fp[0])),
                        ) * 180.0 / std.math.pi));
                        const dx2: f64 = @floatFromInt(cursor[0] - fp[0]);
                        const dy2: f64 = @floatFromInt(cursor[1] - fp[1]);
                        const end_angle: i16 = @intFromFloat(@round(std.math.atan2(-dy2, dx2) * 180.0 / std.math.pi));
                        var sweep: i16 = end_angle - start_angle;
                        if (sweep <= 0) sweep += 360;
                        if (radius_px > 1.0) {
                            h.strokeArc(center, radius_px, start_angle, sweep, thickness, preview_col);
                        }
                    },
                }
            }
        ...
        .polygon => {
            ...
            if (n > 0) {
                ...
                var i: u8 = 0;
                while (i + 1 < n) : (i += 1) {
                    const a = h.w2p(draw.polygon_points[i], vp);
                    const b = h.w2p(draw.polygon_points[i + 1], vp);
                    h.strokeLine(a[0], a[1], b[0], b[1], thickness, preview_col);
                }
                // Draw preview edge from last vertex to cursor.
                const last = h.w2p(draw.polygon_points[n - 1], vp);
                const cur = h.w2p(cursor, vp);
                h.strokeLine(last[0], last[1], cur[0], cur[1], thickness, preview_col);
                // Draw closing edge preview (cursor to first vertex) as dashed (thinner).
                const first = h.w2p(draw.polygon_points[0], vp);
                h.strokeLine(cur[0], cur[1], first[0], first[1], 0.8, Color{
                    .r = preview_col.r, .g = preview_col.g, .b = preview_col.b, .a = 90,
                });
                // Dots on all vertices.
                var j: u8 = 0;
                while (j < n) : (j += 1) {
                    h.strokeDot(h.w2p(draw.polygon_points[j], vp), types.wire_endpoint_radius, preview_col);
                }
            }
        },
        .text => {
            if (draw.text_pos) |pos| {
                const p = h.w2p(pos, vp);
                h.strokeDot(p, types.wire_preview_dot_radius, preview_col);
                if (draw.text_len > 0) {
                    h.drawLabel(draw.text_buf[0..draw.text_len], p[0] + 4, p[1] - 14, preview_col, vp, 96_000);
                } else {
                    h.drawLabel("|", p[0] + 4, p[1] - 14, preview_col, vp, 96_000);
                }
            }
        },
        else => {},
...
pub fn tbIsHovered(app: *st.AppState) bool {
    return app.gui.hot.canvas.tb_overlay.hovered_idx >= 0;
}
...
pub fn tbPreInput(app: *st.AppState, wd: *dvui.WidgetData, bounds: dvui.Rect.Physical) void {
    ...
    for (dvui.events()) |*ev| {
        switch (ev.evt) {
            .mouse => |me| {
                tb_ov.last_mouse_x = me.p.x;
                tb_ov.last_mouse_y = me.p.y;
    ...
    var new_hov: i32 = -1;
    for (0..tbs.len) |idx| {
        if (hitButton(tb_ov.last_mouse_x, tb_ov.last_mouse_y, bounds, idx)) {
            new_hov = @intCast(idx);
            break;
        }
    }
...
// ═══════════════════════════════════════════════════════════════════════════
// Spice2schematic ghost overlay (Phase 4 — testbench SPICE projection)
// ═══════════════════════════════════════════════════════════════════════════

/// Update ghost geometry from simulation raw_output via spice2schematic.
/// Called from tbDraw when the document's sim_generation changes.
/// O(n) parse + layout + route; cached until next sim run.
fn updateGhostFromRawSpice(tb_ov: *TbOverlayCache, raw_spice: []const u8, dut_name: []const u8) void {
    if (raw_spice.len == 0) return;

    // Reset arena for fresh allocation.
    if (tb_ov.ghost_arena) |*a| _ = a.reset(.retain_capacity);
    if (tb_ov.ghost_arena == null) tb_ov.ghost_arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    const arena = tb_ov.ghost_arena.?.allocator();
...
/// Semi-transparent lines for testbench wiring and dot markers for stimulus sources.
/// Read-only: no selection, no editing, no persistence.
pub fn drawGhostOverlay(ctx: *const RenderContext, app: *st.AppState) void {
