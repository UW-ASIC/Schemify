...
fn moveSelectedByMotion(app: *st.AppState, cs: *CanvasState, cur: Vec2, vp: RenderViewport, snap: f32) void {
    ...
    if (doc.selection.lines.bit_length > 0) {
        var it = doc.selection.lines.iterator(.{}); while (it.next()) |idx| { if (idx >= sch.lines.len) continue; sch.lines.items(.x0)[idx] += dx; sch.lines.items(.y0)[idx] += dy; sch.lines.items(.x1)[idx] += dx; sch.lines.items(.y1)[idx] += dy; moved = true; }
    }
    if (doc.selection.rects.bit_length > 0) {
        var it = doc.selection.rects.iterator(.{}); while (it.next()) |idx| { if (idx >= sch.rects.len) continue; sch.rects.items(.x0)[idx] += dx; sch.rects.items(.y0)[idx] += dy; sch.rects.items(.x1)[idx] += dx; sch.rects.items(.y1)[idx] += dy; moved = true; }
    }
    if (doc.selection.circles.bit_length > 0) {
        var it = doc.selection.circles.iterator(.{}); while (it.next()) |idx| { if (idx >= sch.circles.len) continue; sch.circles.items(.cx)[idx] += dx; sch.circles.items(.cy)[idx] += dy; moved = true; }
    }
    if (doc.selection.arcs.bit_length > 0) {
        var it = doc.selection.arcs.iterator(.{}); while (it.next()) |idx| { if (idx >= sch.arcs.len) continue; sch.arcs.items(.cx)[idx] += dx; sch.arcs.items(.cy)[idx] += dy; moved = true; }
    }
    if (doc.selection.texts.bit_length > 0) {
        var it = doc.selection.texts.iterator(.{}); while (it.next()) |idx| { if (idx >= sch.texts.len) continue; sch.texts.items(.x)[idx] += dx; sch.texts.items(.y)[idx] += dy; moved = true; }
    }
...
fn primeMoveIfOnSelected(cs: *CanvasState, app: *st.AppState, mp: Vec2, vp: RenderViewport) void {
    ...
    if (hitTestShape(sch, world_pt)) |sh| {
        const d = app.active() orelse return;
        const bits = switch (sh.kind) {
            .line => &d.selection.lines,
            .rect => &d.selection.rects,
            .circle => &d.selection.circles,
            .arc => &d.selection.arcs,
            .text => &d.selection.texts,
        };
        if (sh.idx < bits.bit_length and bits.isSet(sh.idx)) {
            cs.move_hit_idx = @intCast(sh.idx);
            cs.move_press_pixel = .{ mp[0], mp[1] }; cs.move_start_world = .{ wx, wy }; cs.drag_last = .{ mp[0], mp[1] };
        }
    }
}
...
pub fn hitTestShape(sch: *const Schemify, world: Point) ?ShapeHit {
    const wpx: f64 = @floatFromInt(world[0]);
    const wpy: f64 = @floatFromInt(world[1]);
    const tol2: f64 = @floatFromInt(select_hit_radius_sq);
    ...
    // Arcs (distance to perimeter within sweep)
...
const HitResult = struct { inst_idx: i32, wire_idx: i32, shape: ?ShapeHit = null };
...
fn hitTestCanvas(app: *st.AppState, world: Point) HitResult {
    ...
    if (hitTestShape(sch, world)) |sh| return .{ .inst_idx = -1, .wire_idx = -1, .shape = sh };
