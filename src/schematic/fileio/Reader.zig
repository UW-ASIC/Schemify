...
fn parse(s: *Schemify, input: []const u8, a: Allocator) void {
    ...
    defer {
        if (tt_state.cols) |cols| a.free(cols);
    }
    var gen_state: GenState = .{};
    var plugin_ml: PluginMLState = .{};

    // Multiline accumulators
    var pyspice_buf: List(u8) = .{};
    defer pyspice_buf.deinit(a);
    var doc_buf: List(u8) = .{};
    defer doc_buf.deinit(a);
    var code_buf: List(u8) = .{};
    defer code_buf.deinit(a);
    ...
    while (it.next()) |raw| {
        ...
        if (indent == 0) {
            ...
            if (std.mem.startsWith(u8, trimmed, "SYMBOL ")) {
                s.name = poolAdd(&s.strings, a, std.mem.trim(u8, trimmed["SYMBOL ".len..], " \t"));
            } else if (std.mem.startsWith(u8, trimmed, "TESTBENCH ")) {
                s.name = poolAdd(&s.strings, a, std.mem.trim(u8, trimmed["TESTBENCH ".len..], " \t"));
            } else if (std.mem.startsWith(u8, trimmed, "PLUGIN ")) {
                const pname = std.mem.trim(u8, trimmed["PLUGIN ".len..], " \t");
                s.plugin_blocks.append(a, .{
                    .name = poolAdd(&s.strings, a, pname),
                }) catch {};
                section = .plugin;
            } else if (std.mem.eql(u8, trimmed, "PYSPICE")) {
                section = .pyspice;
            } else if (std.mem.eql(u8, trimmed, "DOCUMENTATION")) {
        ...
        if (indent == 1) {
            if (section == .pyspice) {
                accumulateBuf(a, &pyspice_buf, trimmed);
                continue;
            }
            if (section == .documentation) {
                const content = if (line.len >= 2 and std.mem.eql(u8, line[0..2], "  ")) line[2..] else trimmed;
                accumulateBuf(a, &doc_buf, content);
                continue;
            }
        ...
        switch (section) {
            .plugin_multiline => {
                const raw_content = if (line.len >= 4 and std.mem.eql(u8, line[0..4], "    "))
                    ...
                    trimmed;
                plugin_ml.lines.append(a, a.dupe(u8, raw_content) catch raw_content) catch {};
                continue;
            },
            .pyspice => {
                accumulateBuf(a, &pyspice_buf, trimmed);
            ...
            .code_block => {
                accumulateBuf(a, &code_buf, trimmed);
            },
            .annotations => readAnnotation(a, s, trimmed),
            .generate => {
                gen_state.lines.append(a, a.dupe(u8, line) catch line) catch {};
            },
            .none => {},
        }
    }
    ...
    // Flush multiline buffers into pool
    if (pyspice_buf.items.len > 0)
        s.pyspice_source = poolAdd(&s.strings, a, pyspice_buf.items);
    if (doc_buf.items.len > 0)
        s.documentation = poolAdd(&s.strings, a, doc_buf.items);
    if (code_buf.items.len > 0)
        s.spice_body = poolAdd(&s.strings, a, code_buf.items);
}
...
fn resolveSubSection(trimmed: []const u8, s: *Schemify, a: Allocator, gs: *GenState) ?Section {
    ...
    inline for (kv_meta) |entry| {
        if (std.mem.startsWith(u8, trimmed, entry[0])) {
            ...
            if (val.len > 0) appendProp(a, s, &s.sym_props, entry[1], val);
            return .none;
        }
    }
    ...
    if (std.mem.startsWith(u8, trimmed, "type:")) {
        ...
        appendProp(a, s, &s.sym_props, "symbol_type", val);
        return .none;
    }
...
fn readParam(a: Allocator, s: *Schemify, trimmed: []const u8) void {
    ...
    if (key.len == 0) return;
    appendProp(a, s, &s.sym_props, key, val);
}
...
fn readInstance(a: Allocator, s: *Schemify, trimmed: []const u8) void {
    ...
    while (tok.next()) |kv| {
        ...
        else if (std.mem.eql(u8, key, "name") or std.mem.eql(u8, key, "sym"))
            {}
        else
            s.props.append(a, .{
                .key = poolAdd(&s.strings, a, key),
                .val = poolAdd(&s.strings, a, val),
            }) catch {};
    }

    s.instances.append(a, .{
        .name = poolAdd(&s.strings, a, inst_name),
        .symbol = poolAdd(&s.strings, a, symbol),
...
fn readInstanceParameters(a: Allocator, s: *Schemify, trimmed: []const u8) void {
    ...
    while (pos < body.len) {
        ...
        s.props.append(a, .{ .key = poolAdd(&s.strings, a, key), .val = poolAdd(&s.strings, a, val) }) catch {};
    }
...
fn readNet(a: Allocator, s: *Schemify, trimmed: []const u8, tagged: *List(TaggedConn)) void {
    ...
    const nn_ref = poolAdd(&s.strings, a, nn);
    ...
    while (pin_it.next()) |pin_ref| {
        ...
        tagged.append(a, .{
            ...
            .pin = poolAdd(&s.strings, a, pin_name),
            .net = nn_ref,
        }) catch {};
    }
}
...
fn readInclude(a: Allocator, s: *Schemify, trimmed: []const u8) void {
    ...
    appendProp(a, s, &s.sym_props, "include", trimmed);
}
...
fn readPrefixedProp(a: Allocator, s: *Schemify, trimmed: []const u8, prefix: []const u8) void {
    ...
    defer a.free(pk);
    appendProp(a, s, &s.sym_props, pk, val);
}
...
fn readPluginEntry(a: Allocator, s: *Schemify, trimmed: []const u8, ml: *PluginMLState, section: *Section) void {
    ...
    if (std.mem.eql(u8, val, "|")) {
        ml.key = a.dupe(u8, key) catch key;
        ml.lines = .{};
...
fn flushPluginML(a: Allocator, s: *Schemify, ml: *PluginMLState) void {
    ...
    const pb = &s.plugin_blocks.items[s.plugin_blocks.items.len - 1];
    pb.entries.append(a, .{
        .key = poolAdd(&s.strings, a, ml.key),
        .val = poolAdd(&s.strings, a, joined),
    }) catch {};
    ml.key = "";
    ml.lines = .{};
}
...
fn parseTypeTableHeader(a: Allocator, trimmed: []const u8, state: *TypeTableState) void {
    if (state.cols) |c| a.free(c);
    const delim = for (trimmed, 0..) |c, i| {
        if (c == ' ' or c == '[') break i;
    } else trimmed.len;
    state.type_name = trimmed[0..delim];
...
fn readTypeTableRow(a: Allocator, s: *Schemify, trimmed: []const u8, state: *TypeTableState) void {
    ...
    while (tok.next()) |val| {
        ...
        if (std.mem.eql(u8, col, "name"))
            inst_name = poolAdd(&s.strings, a, val)
            ...
            s.props.append(a, .{ .key = poolAdd(&s.strings, a, col), .val = poolAdd(&s.strings, a, prop_val) }) catch {};
        }
        col_idx += 1;
    }
    ...
    s.instances.append(a, .{
        .name = inst_name,
        .symbol = poolAdd(&s.strings, a, state.type_name),
        .kind = state.kind,
        .x = inst_x,
        .y = inst_y,
        .flags = .{ .rot = inst_rot, .flip = inst_flip },
...
fn repackConns(a: Allocator, s: *Schemify, tagged: []const TaggedConn) void {
    ...
    // Note: conn_start/conn_count are no longer on Instance.
    // Tagged conns from the file are stored for connectivity resolution later.
    // For now, we don't repack into Instance — connectivity.resolve() handles this.
    _ = s;
}
...
fn collapseBusPins(s: *Schemify, a: Allocator) void {
    ...
    for (0..n) |i| {
        ...
        const name_i = s.strings.get(pnames[i]);
        if (splitBusPin(name_i)) |parts| {
            ...
            for (0..n) |j| {
                ...
                const pj = splitBusPin(s.strings.get(pnames[j])) orelse continue;
            ...
            if (count == width and width > 1) {
                ...
                defer a.free(bus_name);
                new_pins.append(a, .{
                    .name = poolAdd(&s.strings, a, bus_name),
                    .x = pxs[i], .y = pys[i], .dir = pdirs[i],
                    .width = @intCast(width),
                }) catch continue;
                continue;
            }
...
fn synthesizePortInstances(s: *Schemify, a: Allocator) void {
    ...
    for (0..s.pins.len) |pi| {
        ...
        const pin_name = s.strings.get(pnames[pi]);
        var found = false;
        for (0..orig_len) |ii| {
            ...
            for (props) |p| {
                if (std.mem.eql(u8, s.strings.get(p.key), "lab") and
                    std.mem.eql(u8, s.strings.get(p.val), pin_name))
                {
                    found = true;
                    break;
                }
        ...
        for (0..s.wires.len) |wi| {
            if (wnn[wi].isEmpty()) continue;
            if (wx0[wi] == wx1[wi] and wy0[wi] == wy1[wi] and
                std.mem.eql(u8, s.strings.get(wnn[wi]), pin_name))
            {
                x = wx0[wi];
                y = wy0[wi];
                break;
        ...
        const prop_start: u32 = @intCast(s.props.items.len);
        s.props.append(a, .{
            .key = poolAdd(&s.strings, a, "lab"),
            .val = pnames[pi],
        }) catch continue;
        s.instances.append(a, .{
            .name = pnames[pi],
            .symbol = poolAdd(&s.strings, a, @tagName(kind)),
            .x = x,
            .y = y,
            .kind = kind,
            .prop_start = prop_start,
...
fn findInstance(s: *Schemify, name: []const u8) ?usize {
    ...
    for (names, 0..) |n, i| {
        if (std.mem.eql(u8, s.strings.get(n), name)) return i;
    }
    return null;
}
...
fn parseColumnNames(a: Allocator, header_rest: []const u8) ?[]const []const u8 {
    ...
    while (col_it.next()) |col| cols.append(a, col) catch {
        cols.deinit(a);
        return null;
    };
    return cols.toOwnedSlice(a) catch {
        cols.deinit(a);
        return null;
    };
}
...
fn poolAdd(pool: *StringPool, a: Allocator, s: []const u8) StringRef {
    return pool.add(a, s) catch .empty;
}
