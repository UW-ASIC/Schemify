...
pub fn findProp(pool: *const StringPool, props: []const Property, key: []const u8) ?[]const u8 {
    for (props) |p| if (std.mem.eql(u8, pool.get(p.key), key)) return pool.get(p.val);
    return null;
}
