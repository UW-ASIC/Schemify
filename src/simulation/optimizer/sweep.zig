...
/// Callback for external simulation (SPICE integration).
...
pub const SweepEngine = struct {
    ...
    observations: [max_evaluations]Individual = undefined,
    ...
    fn evaluate(self: *SweepEngine, x: []const f64) void {
        ...
        if (obs.isFeasible()) {
            ...
            var obj_sum: f64 = 0.0;
            for (obs.objectives[0..obs.n_objectives]) |o| obj_sum += o;
    ...
    fn evaluateAnalytical(self: *SweepEngine, x: []const f64, obs: *Individual) void {
        const specs = self.problem.specs.slice();
        var obj_idx: u32 = 0;
        var con_idx: u32 = 0;

        // For each transistor, compute metrics from its gm/Id value
        for (self.problem.mosfets.slice(), 0..) |t, ti| {
            if (ti >= x.len) break;
            const gmid_val = x[ti];
    ...
    pub fn bestIndividual(self: *const SweepEngine) ?*const Individual {
...
fn eqlAny(needle: []const u8, haystack: []const []const u8) bool {
    for (haystack) |h_item| {
        if (std.ascii.eqlIgnoreCase(needle, h_item)) return true;
    }
    return false;
}
...
test "SweepEngine: analytical optimization" {
    ...
    try std.testing.expect(engine.n_observations > 0);
}
