//! Simulation command handlers — run simulation, open waveform viewer.
...
pub fn handleRunSim(p: types.RunSim, state: anytype) void {
    ...
    if (!analysis_exists) {
        ...
        const pyspice_code = @import("core").simulation.Netlist.emitPySpice(&fio.sch, alloc, null) catch null;
    ...
    const results = @import("core").simulation.results;
    switch (term) {
        .Exited => |code| {
            if (code == 0) {
                fio.sim_results = .{
...
fn regenerateAboveMarker(alloc: std.mem.Allocator, analysis_path: []const u8, sch: anytype) void {
    const Netlist = @import("core").simulation.Netlist;
    const platform_fs = @import("utility").platform.fs;
