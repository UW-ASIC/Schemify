...
pub fn configDir() []const u8 {
    if (comptime is_wasm) return ".config/Schemify";
...
pub fn openUrl(alloc: std.mem.Allocator, url: []const u8) !void {
    if (comptime is_wasm) {
        const open_url_js = struct {
            extern "host" fn platform_open_url(ptr: [*]const u8, len: u32) void;
        }.platform_open_url;
        open_url_js(url.ptr, @intCast(url.len));
        return;
    }
    _ = .{alloc}; // suppress unused on WASM-only path; native uses it below
