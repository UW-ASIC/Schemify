...
fn handleDesignAmplifier(a: std.mem.Allocator, args: ?std.json.Value) []const u8 {
    ...
    const system_text =
        ...
        \\Use the MCP tools (place_component, add_wire, set_property, create_from_topology) to build the circuit.
        \\Read resources (schemify://info, schemify://instances, schemify://skills/core) for context.
    ;
    ...
    const user_text = std.fmt.allocPrint(a,
        ...
        \\Use `create_from_topology` with the designed component values.
...
fn handleImportXschem(a: std.mem.Allocator, args: ?std.json.Value) []const u8 {
    ...
    const system_text =
        ...
        \\Use file I/O tools (read_file, list_project_files, write_file) and circuit tools.
    ;
    ...
    const user_text = std.fmt.allocPrint(a,
        ...
        \\3. Map each component to Schemify using `place_component` or `create_from_topology`
...
fn handleOptimizeSizing(a: std.mem.Allocator, args: ?std.json.Value) []const u8 {
    ...
    const system_text =
        ...
        \\Use circuit tools to read the current design and apply optimized sizes.
    ...
    const user_text = std.fmt.allocPrint(a,
        ...
        \\
        \\### Step 4: Apply Sizes
        \\For each optimized device, use `set_property` to update:
        \\- W (channel width)
        \\- L (channel length)
...
fn handleDesignCurrentMirror(a: std.mem.Allocator, args: ?std.json.Value) []const u8 {
    ...
    const user_text = std.fmt.allocPrint(a,
        ...
        \\
        \\3. **Build**: Use `create_from_topology` with the components and net connections.
