//! MCP diagnostics — circuit validation, connectivity analysis, DRC, and netlist.
//!
//! All core analysis functions operate on `*const Schemify` (the schematic model
//! from `src/core/Schemify.zig`).  Higher-level wrappers accept `*anyopaque`
//! context pointers for MCP tool integration — the opaque pointer is expected to
//! be a `*SchematicRef`.
//!
//! The schematic **must** have had `resolveNets()` called before any diagnostics
//! function that inspects connectivity (connectivity, unrouted_pins, floating_nets,
//! drc_check, validate_circuit).  If conns/nets are empty and there are instances
//! + wires, the functions return a warning about stale net data.
...
// ═══════════════════════════════════════════════════════════════════════════════
// SchematicRef — lightweight handle passed as *anyopaque from MCP tools
// ═══════════════════════════════════════════════════════════════════════════════

/// Opaque handle that MCP tool handlers construct from AppState.
/// Diagnostics casts `*anyopaque` → `*const SchematicRef`.
pub const SchematicRef = struct {
    sch: *const Schemify,
};
...
// ═══════════════════════════════════════════════════════════════════════════════
// 1. connectivity(instance_name, pin_name) -> {net, connected_nodes}
// ═══════════════════════════════════════════════════════════════════════════════

/// Given an instance name and pin name, return which net the pin is on and
/// what other nodes are connected to that net.
pub fn connectivity(a: Allocator, sch: *const Schemify, instance_name: []const u8, pin_name: []const u8) []const u8 {
    ...
    for (names, 0..) |name, i| {
        if (std.mem.eql(u8, name, instance_name)) {
            inst_idx = i;
            break;
        }
    }
    ...
    if (inst_idx == null) {
        w.writeAll("{\"error\":\"Instance not found\",\"instance\":") catch return "{}";
        jsonStr(w, instance_name);
        w.writeByte('}') catch {};
        return buf.items;
    }

    const idx = inst_idx.?;
    const conn_start: usize = sch.instances.items(.conn_start)[idx];
    const conn_count: usize = sch.instances.items(.conn_count)[idx];

    // Find the pin in this instance's connections
    var net_name: ?[]const u8 = null;
    for (sch.conns.items[conn_start..][0..conn_count]) |conn| {
        if (std.mem.eql(u8, conn.pin, pin_name)) {
            net_name = conn.net;
            break;
        }
    }
    ...
    if (net_name == null) {
        w.writeAll("{\"error\":\"Pin not found on instance\",\"instance\":") catch return "{}";
        jsonStr(w, instance_name);
        w.writeAll(",\"pin\":") catch {};
        jsonStr(w, pin_name);
        // List available pins
        w.writeAll(",\"available_pins\":[") catch {};
        for (sch.conns.items[conn_start..][0..conn_count], 0..) |conn, i| {
    ...
    const target_net = net_name.?;

    // Build result
    w.writeAll("{\"instance\":") catch return "{}";
    jsonStr(w, instance_name);
    w.writeAll(",\"pin\":") catch {};
    jsonStr(w, pin_name);
    w.writeAll(",\"net\":") catch {};
    jsonStr(w, target_net);

    // Find all other nodes connected to this net
    w.writeAll(",\"connected_nodes\":[") catch {};
    var first = true;
    const all_names = sch.instances.items(.name);
    const all_cs = sch.instances.items(.conn_start);
    const all_cc = sch.instances.items(.conn_count);

    for (0..sch.instances.len) |i| {
        const cs: usize = all_cs[i];
        const cc: usize = all_cc[i];
        for (sch.conns.items[cs..][0..cc]) |conn| {
            if (std.mem.eql(u8, conn.net, target_net)) {
                // Skip the query pin itself
                if (i == idx and std.mem.eql(u8, conn.pin, pin_name)) continue;
                if (!first) w.writeByte(',') catch {};
                first = false;
                w.writeAll("{\"instance\":") catch {};
                jsonStr(w, all_names[i]);
                w.writeAll(",\"pin\":") catch {};
                jsonStr(w, conn.pin);
                w.writeByte('}') catch {};
            }
...
pub fn netlist(a: Allocator, sch: *const Schemify) []const u8 {
    ...
    const NetlistMod = @import("../simulation/Netlist.zig");
...
/// Validate the circuit. ctx must be a *SchematicRef.
pub fn validate(a: Allocator, ctx: *anyopaque) []const u8 {
    const ref: *const SchematicRef = @ptrCast(@alignCast(ctx));
    return validateCircuit(a, ref.sch);
}

/// Check connectivity. ctx must be a *SchematicRef.
/// Returns JSON with unrouted pins and floating nets.
pub fn checkConnectivity(a: Allocator, ctx: *anyopaque) []const u8 {
    const ref: *const SchematicRef = @ptrCast(@alignCast(ctx));
    const sch = ref.sch;

    var buf: std.ArrayList(u8) = .{};
    const w = buf.writer(a);
    w.writeAll("{\"unrouted\":") catch return "{}";
    const unrouted_json = unroutedPins(a, sch);
    extractJsonArrayField(w, a, unrouted_json, "unrouted_pins");

    w.writeAll(",\"floating\":") catch {};
    const floating_json = floatingNets(a, sch);
    extractJsonArrayField(w, a, floating_json, "floating_nets");

    w.writeByte('}') catch {};
    return buf.items;
}

/// Run DRC. ctx must be a *SchematicRef.
pub fn drc(a: Allocator, ctx: *anyopaque) []const u8 {
    const ref: *const SchematicRef = @ptrCast(@alignCast(ctx));
    return drcCheck(a, ref.sch);
}
...
test "connectivity finds correct net and connected nodes" {
    const a = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(a);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    // Add two resistors connected via net "n1"
    _ = try sch.addComponent(a, .{
        .name = "R1",
        .symbol = "resistor",
        .kind = .resistor,
    ...
    const result = connectivity(arena, &sch, "R1", "n");
    try std.testing.expect(std.mem.indexOf(u8, result, "\"net\":\"n1\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, result, "R2") != null);
}

test "connectivity returns error for unknown instance" {
    const a = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(a);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    const result = connectivity(arena, &sch, "X99", "g");
    try std.testing.expect(std.mem.indexOf(u8, result, "\"error\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, result, "Instance not found") != null);
}
...
test "opaque pointer wrapper - validate" {
    const a = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(a);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    var ref = SchematicRef{ .sch = &sch };
    const result = validate(arena, @ptrCast(&ref));
    try std.testing.expect(std.mem.indexOf(u8, result, "\"valid\"") != null);
}

test "opaque pointer wrapper - checkConnectivity" {
    const a = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(a);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    var ref = SchematicRef{ .sch = &sch };
    const result = checkConnectivity(arena, @ptrCast(&ref));
    try std.testing.expect(std.mem.indexOf(u8, result, "\"unrouted\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, result, "\"floating\"") != null);
}

test "opaque pointer wrapper - drc" {
    const a = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(a);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
    ...
    var ref = SchematicRef{ .sch = &sch };
    const result = drc(arena, @ptrCast(&ref));
    try std.testing.expect(std.mem.indexOf(u8, result, "\"violations\"") != null);
}
