...
pub fn parseVerilog(source: []const u8, top_module: ?[]const u8, alloc: Allocator) ParseError!HdlModule {
    ...
    var params = std.ArrayListUnmanaged(HdlParam){};
    defer {
        for (params.items) |p| {
            alloc.free(p.name);
    ...
    // Step 5: Extract pins
    var pins = std.ArrayListUnmanaged(HdlPin){};
    defer {
        for (pins.items) |p| {
            alloc.free(p.name);
...
fn stripVerilogComments(source: []const u8, alloc: Allocator) Allocator.Error![]u8 {
    ...
    try out.ensureTotalCapacity(alloc, source.len);
...
fn parseAnsiPorts(port_list: []const u8, alloc: Allocator, pins: *std.ArrayListUnmanaged(HdlPin)) Allocator.Error!void {
...
fn parseAnsiPortDecl(decl: []const u8, alloc: Allocator, pins: *std.ArrayListUnmanaged(HdlPin)) Allocator.Error!void {
...
fn parseNonAnsiPorts(port_list: []const u8, body: []const u8, alloc: Allocator, pins: *std.ArrayListUnmanaged(HdlPin)) Allocator.Error!void {
    // Step 1: Collect port names from the header port list
    var port_names = std.ArrayListUnmanaged([]const u8){};
    defer port_names.deinit(alloc);
    ...
    while (i < port_list.len) : (i += 1) {
        if (port_list[i] == ',') {
            ...
            if (name.len > 0) try port_names.append(alloc, name);
            start = i + 1;
        }
    }
    const last_name = std.mem.trim(u8, port_list[start..], &std.ascii.whitespace);
    if (last_name.len > 0) try port_names.append(alloc, last_name);
...
fn extractVerilogParams(source: []const u8, alloc: Allocator, params: *std.ArrayListUnmanaged(HdlParam)) Allocator.Error!void {
...
fn extractVhdlGenerics(gen_section: []const u8, alloc: Allocator, params: *std.ArrayListUnmanaged(HdlParam)) Allocator.Error!void {
...
fn extractVhdlPorts(port_section: []const u8, alloc: Allocator, pins: *std.ArrayListUnmanaged(HdlPin)) Allocator.Error!void {
