...
const ExprFuncMap = struct { universal: []const u8, ngspice: []const u8 };
const expr_func_table = [_]ExprFuncMap{
    .{ .universal = "if", .ngspice = "ternary_fcn" },
    .{ .universal = "step", .ngspice = "u" },
    .{ .universal = "min", .ngspice = "min" },
    .{ .universal = "max", .ngspice = "max" },
    .{ .universal = "abs", .ngspice = "abs" },
    .{ .universal = "sqrt", .ngspice = "sqrt" },
    .{ .universal = "exp", .ngspice = "exp" },
    .{ .universal = "ln", .ngspice = "ln" },
    .{ .universal = "log10", .ngspice = "log" },
    .{ .universal = "pow", .ngspice = "pwr" },
};

pub fn translateFunc(comptime name: []const u8) []const u8 {
    for (expr_func_table) |e| if (std.mem.eql(u8, e.universal, name)) return e.ngspice;
    return name;
}
...
// ═════════════════════════════════════════════════════════════════════════════
// Inlined ngspice emission (was backend/ngspice.zig)
// ═════════════════════════════════════════════════════════════════════════════
