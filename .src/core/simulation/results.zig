//! Result processing and derived analysis utilities.
//!
//! Provides post-processing of raw simulation results:
//!   - Waveform arithmetic (gain, phase extraction from complex AC data)
//!   - Measurement extraction (bandwidth, slew rate, settling time, etc.)
//!   - Multi-sweep result aggregation
//!   - Format conversion for waveform viewers
...
pub const SimResult = struct {
    ...
    pub fn waveformCount(self: *const SimResult) usize {
        return self.waveforms.len;
    }

    pub fn totalPoints(self: *const SimResult) usize {
        var total: usize = 0;
        for (self.waveforms) |w| total += w.x_data.len;
        return total;
    }
};
