...
pub const Runtime = struct {
    ...
    pub fn getPanelHtml(self: *Runtime, panel_id: u16) []const u8 {
        if (panel_id >= self.panel_states.items.len) return &.{};
        return self.panel_states.items[panel_id].html;
    }
    ...
    fn callProcessDrawPanel(self: *Runtime, alloc: std.mem.Allocator, p: *LoadedPlugin, panel_id: u16) void {
        ...
        const DrawCtx = struct {
            ...
            fn cb(tag: Tag, payload: []const u8, ctx: @This()) void {
                if (tag == .html_layout) {
                    // [u16 panel_id][u16 html_len][html bytes]
                    if (payload.len < 4) return;
                    const html_len = std.mem.readInt(u16, payload[2..4], .little);
                    if (4 + html_len > payload.len) return;
                    ctx.ps_ptr.html = ctx.a.dupe(u8, payload[4 .. 4 + html_len]) catch return;
                    return;
                }
...
fn sendUnload(p: *LoadedPlugin, alloc: std.mem.Allocator) void {
    ...
    if (callPluginProcess(p, &unload_msg, alloc)) |buf| {
        alloc.free(buf);
    } else |_| {}
}
