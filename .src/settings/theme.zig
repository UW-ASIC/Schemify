//! Theme settings — loading, saving, and applying theme configuration.
//! Reads from ~/.config/Schemify/theme.json and bundled presets.
...
/// Apply a ThemeConfig directly (from JSON editor or programmatic use).
pub fn applyConfig(config: ThemeConfig) void {
...
// ── Convert ThemeConfig to JSON string for theme.applyJson() ────────────────

/// Produce a JSON string suitable for passing to gui/theme.zig's applyJson().
/// This is the bridge between settings and the live theme system.
pub fn configToOverridesJson(config: *const ThemeConfig, a: std.mem.Allocator) ?[]const u8 {
    return serializeConfig(config, a);
}
